...
function [bowTrain, bowTest, entropies] = bowLEclusters(featsTrain, labelTrain, nClasses,...
                                             featsTest, K)
    nTrain  =   size(featsTrain,1);
    nTest   =   size(featsTest,1);
    
    idxTrain = [];
    bofeatsTrain = [];
    labelTrainMat = [];
    for i=1:nTrain
        idxTrain = [idxTrain; i*ones(size(featsTrain{i},1),1)];
        bofeatsTrain  = [bofeatsTrain; cell2mat(featsTrain{i})];
        labelTrainMat = [labelTrainMat; labelTrain(i)*ones(size(featsTrain{i},1),1)];
    end
    ...
    for i=1:nTest
        ibofTest = cell2mat(featsTest{i});
        nSeqs = size(featsTest{i},1);
