...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			...
			for j = 1:length(xdata)
				text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 30, 'fontsize', 30, 'HorizontalAlignment', 'right');
			end
		end
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontsize',40);
	end
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontsize',30);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontsize',30);
	end
	
% 	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', 25, 'xtick',1:numgroups, 'linewidth', 2,'xgrid','off','ygrid','off');
	set(gca, 'xticklabel', groupnames, 'ticklength', [0 0], 'fontsize', 25, 'xtick',1:numgroups, 'linewidth', 2,'xgrid','off','ygrid','off');
