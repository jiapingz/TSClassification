...
int nrhs, const mxArray* prhs[])
{
  ...
  /* Make sure input number is sufficient */
    ...
    const long unsigned int *idims =  mxGetDimensions(DISTMATRIX);
    if(idims[0] != idims[1])
    ...
    if(mxIsSparse(DISTMATRIX)) {   /* Sparse matrix handling */
        ...
        long unsigned int *distmat_rows = mxGetIr(DISTMATRIX);
        long unsigned  int *distmat_cols = mxGetJc(DISTMATRIX);
